...
class RegistrarWindow:
    ...
    def issue_registrar(self):
        if not os.path.exists(cert_manager.CA_KEY_FILE):
            ...
            return
        ca_password = simpledialog.askstring("CA Password", "Enter CA private key password:", show='*')
        if not ca_password:
            return
    ...
    def sign_pdf(self):
        ...
        if not pdf_file:
            return
        sig_file = filedialog.asksaveasfilename(defaultextension=".sig", filetypes=[("Signature files", "*.sig")])
        if not sig_file:
            return
        try:
    ...
    def export_public_cert(self):
        ...
        try:
            ...
            with open(filename, 'wb') as f:
                f.write(pem_data)
            messagebox.showinfo("Success", f"Public certificate exported to {filename}")
        except Exception as e:
